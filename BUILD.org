...
cmigemoを組み込み、ネイティブコンパイル機能が有効なemacsを作成します。スタティックリンクなバイナリになりますので、msys2をインストールしていない別のWindows環境でも、インストールディレクトリを丸ごとコピーするだけで動作します。
...
#+BEGIN_EXAMPLE
wget https://github.com/emacs-mirror/emacs/archive/refs/tags/emacs-31.1.tar.gz
#+END_EXAMPLE

展開します。

#+BEGIN_EXAMPLE
tar xf emacs-31.1.tar.gz
#+END_EXAMPLE

=emacs-emacs-31.1= ディレクトリの下にソースツリーができます。
...
#+BEGIN_EXAMPLE
cd emacs-emacs-31.1
patch -p1 < ../emacs-on-windows-patches/02-emacs-31.1-dontusereg.patch
patch -p1 < ../emacs-on-windows-patches/03-emacs-31.1-cmigemo.patch
patch -p1 < ../emacs-on-windows-patches/04-emacs-31.1-mingw32-remove-check.patch
patch -p1 < ../emacs-on-windows-patches/001-ucrt-31.1.patch
...
#+BEGIN_EXAMPLE
cd emacs
patch -p1 < ../emacs-on-windows-patches/02-emacs-20260825-dontusereg.patch
patch -p1 < ../emacs-on-windows-patches/03-emacs-20260825-cmigemo.patch
patch -p1 < ../emacs-on-windows-patches/04-emacs-20260825-mingw32-remove-check.patch
patch -p1 < ../emacs-on-windows-patches/001-ucrt-20260825.patch
patch -p1 < ../emacs-on-windows-patches/manifest-utf8.patch
#+END_EXAMPLE
...
#+BEGIN_EXAMPLE
wget http://ftp.iij.ad.jp/pub/sourceforge.jp/nkf/70406/nkf-2.1.5.tar.gz
...
#+BEGIN_EXAMPLE
cd cmigemo/dict
wget https://github.com/skk-dev/dict/raw/refs/heads/master/SKK-JISYO.L
#+END_EXAMPLE
...
cmake -B build
cmake --build build
cmake --install build --component core --prefix /usr/local
cmake --install build --component dict --prefix /usr/local
#+END_EXAMPLE

=/usr/local/bin/cmigemo.exe= 、 =/usr/local/bin/libmigemo.dll= 、 =/usr/local/include/migemo.h= などがインストールされます。
