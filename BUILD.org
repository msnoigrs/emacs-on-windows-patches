...
#+BEGIN_EXAMPLE
wget https://github.com/emacs-mirror/emacs/archive/refs/tags/emacs-30.2.tar.gz
#+END_EXAMPLE

展開します。

#+BEGIN_EXAMPLE
tar xf emacs-30.2.tar.gz
#+END_EXAMPLE

=emacs-emacs-30.2= ディレクトリの下にソースツリーができます。
...
#+BEGIN_EXAMPLE
cd emacs-emacs-30.2
patch -p1 < ../emacs-on-windows-patches/02-emacs-30.2-dontusereg.patch
patch -p1 < ../emacs-on-windows-patches/03-emacs-30.2-cmigemo.patch
patch -p1 < ../emacs-on-windows-patches/04-emacs-30.2-mingw32-remove-check.patch
patch -p1 < ../emacs-on-windows-patches/001-ucrt-30.2.patch
...
dllwrapがdeprecatedになったので、gcc -sharedを利用するようにするパッチを適用します。

#+BEGIN_EXAMPLE
cd ../cmigemo
patch -p1 < ../emacs-on-windows-patch/cmigemo-make-dll.patch
